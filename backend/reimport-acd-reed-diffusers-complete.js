...
const results = [];
const csvFilePath = path.join(__dirname, 'aw-acd-ds-import.csv');
...
// Rimuovi vecchi reed diffusers ACD
products = products.filter(p => !p.sku || !p.sku.startsWith('ACD-'));
layout.sidebar = layout.sidebar.filter(id => !id || !id.startsWith('ACD-'));

console.log('📦 Importazione COMPLETA Reed Diffusers da AW Dropship\n');
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    ...
    console.log(`   - ${imported} reed diffusers importati`);
