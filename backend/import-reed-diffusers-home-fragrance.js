...
const results = [];
const csvFilePath = path.join(__dirname, 'aw-reed-diffusers-home-fragrance.csv');
...
// Rimuovi eventuali vecchi Reed Diffusers RDHF (se esistono)
const oldCount = products.length;
products = products.filter(p => !p.sku || !p.sku.startsWith('RDHF-'));
const removed = oldCount - products.length;

// Rimuovi anche dal layout
layout.sidebar = layout.sidebar.filter(id => !id || !id.startsWith('RDHF-'));

console.log('📦 Importazione Reed Diffusers Home Fragrance da AW Dropship');
if (removed > 0) {
  console.log(`🗑️  Rimossi ${removed} vecchi Reed Diffusers Home Fragrance\n`);
}
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    console.log(`✅ Letti ${results.length} Reed Diffusers dal CSV\n`);
    ...
    results.forEach((row) => {
      ...
      const product = {
        ...
        zenovaSubcategory: 'oli-fragranza',
        weight: weight,
    ...
    console.log(`   - ✅ ${imported} Reed Diffusers Home Fragrance importati`);
