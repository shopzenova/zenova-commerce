...
const CSV_FILE = './pietre-preziose-NEW.csv';
const BACKUP_FILE = `./top-100-products.backup-before-pietre-preziose-import-${Date.now()}.json`;

console.log('📦 IMPORT PIETRE PREZIOSE\n');
...
fs.createReadStream(CSV_FILE)
  ...
  .on('end', () => {
    console.log(`\n✅ CSV letto: ${csvData.length} pietre preziose\n`);
    ...
    csvData.forEach(row => {
      ...
      const description = descriptionPlain || `Pietra preziosa di alta qualità. ${name}. Perfetta per cristalloterapia, meditazione e decorazione.`;
      ...
      const newProduct = {
        ...
        inStock: stock > 0,
        featured: false,
        tags: ['pietra preziosa', 'cristallo', 'gemma', 'wellness', 'meditazione', 'cristalloterapia'],
        source: 'aw',
        supplier: 'AW',
        awId: sku,
        rating: 4.5,
    ...
    console.log(`   Importati pietre preziose: ${importedCount}`);
