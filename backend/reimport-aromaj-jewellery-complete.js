...
const results = [];
const csvFilePath = path.join(__dirname, 'aw-gel-gemstones-4.csv');
...
// Rimuovi vecchi prodotti AromaJ (Aromatherapy Jewellery)
products = products.filter(p => !p.sku || !p.sku.startsWith('AromaJ-'));
layout.sidebar = layout.sidebar.filter(id => !id || !id.startsWith('AromaJ-'));

console.log('📦 Importazione COMPLETA Aromatherapy Jewellery da AW Dropship\n');
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    ...
    results.forEach((row) => {
      ...
      // Skip prodotti non attivi
      if (status !== 'Active') {
        return;
      }

      // Skip prodotti GEL (già importati)
      if (sku.startsWith('GEL-')) {
        console.log(`   ⚠️  ${sku} è un Gemstone Light, già importato - skip`);
        skipped++;
        return;
      }
    ...
    console.log(`   - ${imported} Aromatherapy Jewellery importati`);
