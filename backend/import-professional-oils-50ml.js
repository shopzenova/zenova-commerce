...
const results = [];
const csvFilePath = path.join(__dirname, 'aw-professional-oils.csv');
...
// Rimuovi eventuali vecchi oli professionali PrEO (se esistono)
const oldCount = products.length;
products = products.filter(p => !p.sku || !p.sku.startsWith('PrEO-'));
const removed = oldCount - products.length;

// Rimuovi anche dal layout
layout.sidebar = layout.sidebar.filter(id => !id || !id.startsWith('PrEO-'));

console.log('📦 Importazione Oli Essenziali Professionali 50ml da AW Dropship');
if (removed > 0) {
  console.log(`🗑️  Rimossi ${removed} vecchi oli professionali\n`);
}
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    console.log(`✅ Letti ${results.length} oli professionali dal CSV\n`);
    ...
    console.log(`   - ✅ ${imported} oli professionali 50ml importati`);
