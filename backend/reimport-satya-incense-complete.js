...
const results = [];
const csvFilePath = path.join(__dirname, 'aw-incense.csv');
...
// Rimuovi vecchi prodotti iSatya (Satya Incense)
products = products.filter(p => !p.sku || !p.sku.startsWith('iSatya-'));
layout.sidebar = layout.sidebar.filter(id => !id || !id.startsWith('iSatya-'));

console.log('📦 Importazione COMPLETA Satya Incense da AW Dropship\n');
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', () => {
    ...
    console.log(`   - ${imported} Satya Incense importati`);
