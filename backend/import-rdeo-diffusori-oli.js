...
const CSV_PATH = 'C:\\Users\\giorg\\Downloads\\portfolio_data_feed_cdeu-000774-dssk_20251220.csv';
const PRODUCTS_FILE = './top-100-products.json';
const BACKUP_FILE = `./products.backup-import-rdeo-${Date.now()}.json`;

console.log('📦 Importazione Reed Diffusers (Diffusori Oli) da AW...\n');
...
fs.createReadStream(CSV_PATH)
    ...
    .on('data', (row) => {
        ...
        const product = {
            ...
            tags: ['AW Dropship', 'Reed Diffusers', 'Essential Oils', 'Diffusori a Bastoncini'],
            brand: 'AW Dropship',
            supplier: 'AW',
            barcode: row['Barcode'] || '',
    ...
    .on('end', () => {
        ...
        const low = newProducts.filter(p => p.stockStatus === 'Low').length;
