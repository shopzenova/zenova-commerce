...
const CSV_FILE = './diffusori-oli-NEW.csv';
const BACKUP_FILE = `./top-100-products.backup-before-diffusori-oli-import-${Date.now()}.json`;

console.log('📦 IMPORT DIFFUSORI OLI\n');
...
fs.createReadStream(CSV_FILE)
  ...
  .on('end', () => {
    console.log(`\n✅ CSV letto: ${csvData.length} prodotti diffusori oli\n`);
    ...
    csvData.forEach(row => {
      ...
      const newProduct = {
        ...
        sku: sku,
        name: name,
        description: `Diffusore oli essenziali di alta qualità. ${name}. Perfetto per profumare naturalmente i tuoi ambienti con eleganza.`,
        price: rrpPrice,
        ...
        inStock: stock > 0,
        featured: false,
        tags: ['diffusore', 'oli essenziali', 'wellness', 'profumo', 'reed diffuser'],
        source: 'aw',
        supplier: 'AW',
        awId: sku,
        rating: 4.5,
    ...
    console.log(`   Importati diffusori oli: ${importedCount}`);
